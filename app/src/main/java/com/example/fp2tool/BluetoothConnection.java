...
public class BluetoothConnection {
    ...
    private static final String NAME = "BTLuminar";
    ...
    public synchronized void connect(BluetoothDevice device) {
        ...
        Log.d("Connect", "Finalmente STATE_CONNECTING");
    }
...
/*            try {
        	...
        	try {
        	   ...
        	   Method mm = device.getClass().getMethod("setPin", byte[].class);
        	   Method convert = device.getClass().getMethod("convertPinToBytes", String.class);
        	   byte[] pin = (byte[]) convert.invoke(device, "000000");
        	   mm.invoke(device,  pin);
            ...
            */
            Method m = null;
            try{
            	Log.i(TAG, "ConnectThread: Intento createRfcommSocketToServiceRecord");
            	tmp = device.createRfcommSocketToServiceRecord(MY_UUID);
            } catch ( IOException exception ){
				try {
					Log.i(TAG, "ConnectThread: Pruebo otra cosa porque lo anterior no ha ido"); 
        ...
        public void run() {
            ...
            try {
            	...
            	Log.e(TAG, "run(): connection failed");
                connectionFailed();
                // Close the socket
                try {
                    mmSocket.close();
